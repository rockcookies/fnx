...
describe('Events', function() {
  ...
  it('bind a callback with a supplied context', function() {
    ...
    obj.trigger('event')
    expect(spy.calledOn(context)).toBe(true);
  })
  ...
  it('remove all events for a specific callback', function() {
    ...
    expect(success.callCount).toEqual(4)
    expect(fail.callCount).toEqual(0)
  })
  ...
  it('off is chainable', function() {
    ...
    expect(obj.off('event')).toEqual(obj)
  })

  // 百年难得一遇，不考虑
  xit('no DontEnums bug', function() {
    var obj = new Events()
    var counter = 0
  ...
  // 非常稀有的场景，不值得
  xit('hasOwnProperty is ok', function() {
    var obj = new Events()
    var counter = 0
  ...
  xit('ignore exception in callback list', function() {
    ...
    spy3 && expect(spy3.calledWith(err.stack)).toBe(true)
  })
  ...
  it('trigger arguments', function() {
    ...
    obj.on('all', spy2)
    obj.trigger('a', 1, 2, 3)
    expect(spy1.calledWith(1, 2, 3)).toBe(true)
    expect(spy2.calledWith('a', 1, 2, 3)).toBe(true)
  })
  ...
  it('#8 mixTo', function() {
   ...
   expect(foo.hasOwnProperty('on')).toBe(true);
  })
  ...
  it('once', function() {
    ...
    obj.trigger('a', 1)
    obj.trigger('a', 1)
    expect(spy1.withArgs(1).calledOnce).toBe(true)
    expect(spy1.calledOn(context)).toBe(true)
    expect(spy2.withArgs('b', 1).calledOnce).toBe(true)
    expect(spy2.calledOn(context)).toBe(true)
  })
